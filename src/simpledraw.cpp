...
//LIST objectList;  // Assuming LIST is your doubly linked list type;



//Tests
void testRectangle(int x1, int y1, int x2, int y2) {
    glColor3f(1, 0, 0);
    glBegin(GL_QUADS);
    glVertex2i(x1, y1);
    glVertex2i(x2, y1);
...
void printList(LIST *list) {
    ...
    // Traverse the list and print each shape
...
void init(void) {
	...
	glutCreateWindow("SimpleDraw Aman Imran");
		...
		if (button == GLUT_LEFT_BUTTON) {
			...
			if (action == GLUT_DOWN) {
				...
				if (objType == RECTANGLE) {
                    ...
                    // Initialize circle (x2, y2) as another point on the circumference
                ...
                SHAPE* newObject = (SHAPE*)malloc(sizeof(SHAPE));
                *newObject = tempObj;
				// and the new object to the object list
                NODE* newNode = createNode(newObject);
                insert(&objlist, newObject);
				printList(&objlist);
...
void mouseMotionFcn(GLint xMouse, GLint yMouse) {
	...
	if (isInDraw) {
	        tempObj.x2 = xMouse;  // Update the second point of the rectangle
	        tempObj.y2 = yMouse;  // Update the second point of the rectangle
	    }
